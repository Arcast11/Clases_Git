...
class GRAFO {
 private:
  ...
  vector<LA_nodo>
      LS;  // Lista de adyacencia de los sucesores: es una matriz dode cada fila
           // posiblemente es posiblemente de distinto tamano
  vector<LA_nodo>
      LP;  // Lista de adyacencia de los predecesores: es una matriz dode cada
           // fila posiblemente es posiblemente de distinto tamano
  void destroy();  // Destructor del LS, y, en su caso, LP
  void build(char nombrefichero[85],
             int& errorapertura);  // Crea LS, A y, en su caso, LP, desde la
                                   // lectura del fichero
  void dfs_cc(unsigned i,
              vector<bool>& visitado);  // Recorrido en profundidad recursivo
                                        // con solo marca de visitado para el
                                        // calculo de las componentes conexas
  void dfs_postnum(unsigned i, vector<bool>& visitado,
                   vector<unsigned>& postnum,
                   unsigned& postnum_ind);  // Recorrido en profundidad
                                            // recursivo para calcular postnum
  void dfs_cfc(
      unsigned i,
      vector<bool>& visitado);  // Recorrido en profundidad recursivo con solo
                                // marca de visitado para el calculo de las
