...
sparse_vector_t::sparse_vector_t(const vector_t<double>& v, const double eps)
    : pv_(), nz_(0), n_(0) {
  // Primer bucle: contamos el número de valores no nulos
  ...
  // Redimencionamos el vector pv_ (vector par: valor + indice)
  pv_.resize(nz_);

  // Le damos valor 0 a nz para realizar el segundo bucle
  nz_ = 0;

  // Segundo bucle: Asigna el valor al vector de pares
  for (size_t i{0}; i < v.get_size(); i++) {
    if (IsNotZero(v[i])) {
      pv_[nz_].set(v[i], i);
      nz_++;
