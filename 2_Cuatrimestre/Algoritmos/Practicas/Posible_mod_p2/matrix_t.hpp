...
void matrix_t<T>::multiply(const matrix_t<T>& A, const matrix_t<T>& B) {
  // Comprobación de que se puede realizar la multiplicación
  assert(A.get_n() == B.get_m());
  // Tamaño de la matriz resultante
  resize(A.get_m(), B.get_n());

  // Avanza las filas de la matriz
  for (size_t i{1}; i <= A.get_m(); i++) {
    // Recorre las columnas de la matriz
    for (size_t j{1}; j <= B.get_n(); j++) {
      // Me aseguro que cada elemento de la matriz resultado empieze en '0'
      at(i, j) = 0;
