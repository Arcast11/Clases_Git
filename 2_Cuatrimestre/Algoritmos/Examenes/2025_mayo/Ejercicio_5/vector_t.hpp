...
class vector_t {
 public:
  ...
  // Examen
  double cos_sim(const vector_t<double>&);
...
// Examen
template <class T>
double vector_t<T>::cos_sim(const vector_t<double>& B) {
  assert(get_size() == B.get_size()); // deben medir lo mismo
  ...
  for (size_t i = 0; i < B.get_size(); i++) {
    ...
    num += at(i) * B.at(i);
    den += sqrt(pow(a, 2)) * sqrt(pow(b, 2));
  }
  // para evitar errores cuando es 0
  if (num * den == 0) {
    return 0.0;
  } else {
    similarity = num / den;
